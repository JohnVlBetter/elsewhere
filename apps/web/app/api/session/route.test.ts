...
describe("POST /api/session", () => {
  ...
  it("creates a selected pack session and returns generic state", async () => {
    ...
    expect(body.intro).toContain("你进入《");
    expect(body.intro).not.toContain("浣犺繘鍏");
    expect(body.intro).not.toContain("鐩爣");
  });

  it("returns readable copy for a missing pack", async () => {
    vi.resetModules();
    process.env.AIGAME_SESSION_ROOT = mkdtempSync(join(tmpdir(), `session-route-${randomUUID()}-`));
    ...
    const response = await POST(new Request("http://test.local/api/session", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ packId: "missing-pack" })
    }));
    const body = await response.json();

    expect(response.status).toBe(404);
    expect(body.error).toBe("没有找到这个故事。");
    expect(body.error).not.toContain("鏈");
  });
});
